...
from AC_Transformer._1b_config import (
    INPUT_DIM, TRANSFORMER_DIM, NUM_HEADS, NUM_LAYERS, FF_DIM, DROPOUT,
    LEARNING_RATE, WEIGHT_DECAY, EPOCHS, PATIENCE, OUTPUT_DIR, RANDOM_SEED, PREDICT_HORIZON
)
...
def plot_training_history(history, save_path):
    ...
    plt.title(f'Transformer 5年预测训练过程 - 损失曲线', fontproperties=fm.FontProperties(family=CHINESE_FONT))
...
def train_model(train_loader, val_loader, device=None):
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            best_model_path = os.path.join(OUTPUT_DIR, 'transformer_5yr_best_model.pth')
            torch.save({
                'epoch': epoch,
    ...
    plot_path = os.path.join(OUTPUT_DIR, 'transformer_5yr_training_history.png')
...
if __name__ == '__main__':
    from AC_Transformer._2b_sequence_builder import load_and_build_sequences, split_data, create_data_loaders

    print("=" * 60)
    print(f"Transformer 5年预测训练 (PREDICT_HORIZON={PREDICT_HORIZON})")
    print("=" * 60)
    ...
    from AC_Transformer._6b_predictor import evaluate_model_from_loader
    test_metrics = evaluate_model_from_loader(
        model, test_loader, device,
        save_path=os.path.join(OUTPUT_DIR, 'transformer_5yr_test_evaluation_report.md')
    )
    print(f"\n[5年预测] 测试集最终评估结果:")
