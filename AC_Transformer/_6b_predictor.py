"""
Transformer 5年预测器和评估器
扩展实验：用前5年数据预测未来第5年

与基础预测器的差异：
- 使用 _1b_config.py 配置
- 加载 transformer_5yr_best_model.pth
- 输出文件添加 _5yr 后缀
...
from AC_Transformer._1b_config import OUTPUT_DIR, PREDICT_HORIZON
...
def evaluate_model(predictions, actuals, save_path=None):
    ...
    if save_path:
        ...
        md_content = f"""# Transformer 5年预测模型评估报告
...
def plot_predictions(predictions, actuals, save_path=None):
    ...
    plt.title(f'Transformer {PREDICT_HORIZON}-Year Ahead Prediction: Predicted vs Actual')
...
def plot_residuals(predictions, actuals, save_path=None):
    ...
    axes[0].set_title(f'Transformer {PREDICT_HORIZON}-Year Residual Distribution')
...
def main():
    """主评估流程"""
    print("=" * 60)
    print(f"Transformer {PREDICT_HORIZON}年预测模型评估")
    print("=" * 60)

    # 加载数据
    from AC_Transformer._2b_sequence_builder import main as build_sequences
    _, _, test_loader, _ = build_sequences()

    # 加载模型
    from AC_Transformer._3_transformer_model import TransformerModel
    from AC_Transformer._1b_config import INPUT_DIM, TRANSFORMER_DIM, NUM_HEADS, NUM_LAYERS, FF_DIM, DROPOUT

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_path = os.path.join(OUTPUT_DIR, 'transformer_5yr_best_model.pth')
    ...
    metrics = evaluate_model(
        predictions, actuals,
        save_path=os.path.join(OUTPUT_DIR, 'transformer_5yr_evaluation_report.md')
    )

    # 绘图
    plot_predictions(
        predictions, actuals,
        save_path=os.path.join(OUTPUT_DIR, 'transformer_5yr_predictions.png')
    )

    plot_residuals(
        predictions, actuals,
        save_path=os.path.join(OUTPUT_DIR, 'transformer_5yr_residuals.png')
    )
    ...
    results_path = os.path.join(OUTPUT_DIR, 'transformer_5yr_predictions.csv')
