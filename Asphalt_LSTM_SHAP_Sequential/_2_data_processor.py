# -*- coding: utf-8 -*-
# 步骤2：数据生成+预处理+划分数据集（独立可运行，前置：步骤1，与Transformer完全一致）
# 运行成功后：生成中间产物（归一化器、数据集），下一步运行3_lstm_model.py
import numpy as np
import pandas as pd
import joblib
import torch
...
from _1_config import INTERIM_DIR, DATA_DIR
import os

# ---------------------- 加载步骤1的全局参数+校验前置条件 ----------------------
def check_prerequisite():
    """校验步骤1是否运行成功"""
    # 【修改】：使用从 config 导入的 INTERIM_DIR，而不是写死 "./interim/"
    PARAMS_SAVE_PATH = os.path.join(INTERIM_DIR, "step1_params.joblib")
    if not os.path.exists(PARAMS_SAVE_PATH):
        print("错误！未找到步骤1的中间产物，请先运行1_config.py")
        exit(1)
    # 加载全局参数
...
def generate_and_process_data(SAMPLE_NUM, SEQ_LEN, INPUT_DIM):
    """生成数据+预处理+划分数据集（与Transformer版本完全一致）"""
    # 1. 生成静态配方/工况特征（与Transformer版本完全一致）
    oil_stone_ratio = np.random.uniform(3.5, 5.5, SAMPLE_NUM)    # 扩大范围：3.5% - 5.5%
    void_ratio = np.random.uniform(2.0, 6.0, SAMPLE_NUM)         # 扩大范围：2% - 6%
    mineral_powder = np.random.uniform(3.0, 8.0, SAMPLE_NUM)     # 扩大范围：3% - 8%
    asphalt_type = np.random.randint(0, 3, SAMPLE_NUM)           # 增加类型：0, 1, 2
    aggregate_grade = np.random.randint(0, 3, SAMPLE_NUM)        # 增加级配类型
    temperature = np.random.uniform(-10.0, 40.0, SAMPLE_NUM)     # 扩大温度范围：-10℃ - 40℃
    stress = np.random.uniform(0.1, 1.0, SAMPLE_NUM)             # 扩大应力水平：0.1 - 1.0 MPa
    
    # 2. 生成时序劲度模量特征（与Transformer版本完全一致）
    stiffness_data = []
    ...
    # 3. 生成疲劳寿命标签（与Transformer版本完全一致）
    fatigue_life = (
        15 + 
        1.2 * oil_stone_ratio -      # 油石比正向影响
        0.8 * void_ratio -           # 空隙率负向影响增强
        0.1 * temperature -          # 温度负向影响增强
        5.0 * stress +               # 应力负向影响大幅增强
        0.00015 * stiffness_data[:, -1]
    ) + np.random.normal(0, 0.3, SAMPLE_NUM)
    
    # 4. 整合特征（静态特征+时序特征）（与Transformer版本完全一致）
    ...
    # 8. 保存原始数据到data目录（与Transformer版本完全一致）
    df_columns = [
        "oil_stone_ratio", "void_ratio", "mineral_powder",
        "asphalt_type", "aggregate_grade", "temperature", "stress"
    ...
    df.to_csv(os.path.join(DATA_DIR, "asphalt_sample_data.csv"), index=False)
    print(f"成功保存原始数据到：{os.path.join(DATA_DIR, 'asphalt_sample_data.csv')}")
    
    # 9. 保存中间产物到interim目录（供后续步骤读取，与Transformer版本完全一致）
    interim_files = {
        "scaler_X": scaler_X,
        "scaler_y": scaler_y,
