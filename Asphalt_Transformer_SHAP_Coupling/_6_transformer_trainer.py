# -*- coding: utf-8 -*-
# 步骤6：Transformer模型训练+保存最优模型
# 运行成功后：生成模型权重+损失历史，下一步运行5_transformer_evaluator.py
import torch
...
def train_model(params_dict, tensor_data):
    ...
    # --- ！！！手动覆盖为最优参数 (根据你的调优结果) ！！！ ---
    D_MODEL = 64          # 调优后的最优 d_model
    NHEAD = 2             # 调优后的最优 nhead
    NUM_ENCODER_LAYERS = 1 # 调优后的最优 num_encoder_layers
    BATCH_SIZE = 16       # 调优后的最优 batch_size
    LEARNING_RATE = 0.0001 # 调优后的最优 learning_rate
    DROPOUT_RATE = 0.1    # 调优后的最优 dropout
    # ---------------------------------------------------
...
if __name__ == "__main__":
    print("="*50)
    print("步骤6：Transformer模型训练+保存最优模型")
    print("="*50)
    params, tensor_data = check_prerequisite()
    train_model(params, tensor_data)
    print("="*50)
    print("步骤6运行成功！下一步运行：7_transformer_evaluator.py")
    print("="*50)