...
from _1_lstm_ablation_no_climate_config import (
    ...
    BATCH_SIZE, RANDOM_SEED, TRAIN_RATIO, VAL_RATIO, TEST_RATIO, OUTPUT_DIR, EXPERIMENT_NAME
)
...
def load_and_build_sequences():
    """加载数据并构建时序序列"""
    print("=" * 60)
    print(f"消融实验 [{EXPERIMENT_NAME}] - 数据加载")
    print("=" * 60)
...
def main():
    ...
    return train_loader, val_loader, test_loader, scaler, len(train_data[0])


if __name__ == '__main__':
    train_loader, val_loader, test_loader, scaler, n_features = main()
    print(f"\n特征维度: {n_features}")
    print("序列构建完成!")
